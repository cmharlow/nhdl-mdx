...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="//oai_qdc:qualifieddc">
        <oai_qdc:qualifieddc xmlns:oai_qdc="http://worldcat.org/xmlschemas/qdc-1.0/" 
            ...
            <!-- will match specific templates that relevant for p15828coll4. -->
            <xsl:apply-templates />
            ...
            <dcterms:isPartOf>Draper and Maynard (D&amp;M) Sporting Goods</dcterms:isPartOf>
            <!-- remove this when the dcterms:provenance field is added back -->
            <edm:dataProvider>Plymouth State University</edm:dataProvider>
            <xsl:call-template name="hub"/>
        </oai_qdc:qualifieddc>
    </xsl:template>
    
    <!-- This collection should ignore the creator field. --> 
    <xsl:template match="dc:creator" />
    
    <!-- This collection should ignore transcript description fields, so only map 1st. --> 
