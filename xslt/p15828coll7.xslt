...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="//oai_qdc:qualifieddc">
        <oai_qdc:qualifieddc xmlns:oai_qdc="http://worldcat.org/xmlschemas/qdc-1.0/" 
            ...
            <!-- will match specific templates that relevant for p15828coll7. -->
            <xsl:apply-templates />
            ...
            <dcterms:isPartOf>Museum of the White Mountains</dcterms:isPartOf>
            <xsl:call-template name="hub"/>
        </oai_qdc:qualifieddc>
    </xsl:template>
    
    <!-- This collection should ignore measurement notes & subcollection fields.
         There is no determined order, so filtering on words. Cannot find subcollections data. --> 
    <xsl:template match="dc:description">
        <xsl:if test="normalize-space(.)!=''">
            <xsl:if test="starts-with(lower-case(.), 'mount') or starts-with(., 'print mounted')">
    ...
    <!-- Ignore Extents unless it is time duration, e.g., DD:DD:DD or DD:DD.D -->
    <xsl:template match="dcterms:extent">
        <xsl:if test="normalize-space(.)!='' and matches(normalize-space(.), '\d{2}:\d{2}')">
